...
class AdvancedSocialCognition:
    ...
    Implementation of Section 19: Theory of Mind and Recursion.
    Supports the hierarchy M0 (Self), M1 (Other), M2 (Self as seen by Other).
    
    Level 0: Self-model
      M₀(t) = r(X(t), a(t), Θ(t)) (Eq. 958, 960)
      
    Level 1: Other-model
      M₁_other(t) = r_ToM(O_observable(t), Context(t)) (Eq. 970)
      
    Level 2: Self-modeling-other-modeling-self
      M₂_self(t) = "Что он думает обо мне?" (Eq. 984)
    """
    ...
    def update_M0_self(self, my_action_plan: torch.Tensor, val: float, ar: float):
        ...
        Update Self-Model (Ground Truth).
        
        Theory Section 19e, Level 0:
        dM₀/dt = r₀(X_self(t), a_self(t), Body_self(t), Θ_self(t))
        
        M₀ includes:
        - Intentions (a_self)
        - Emotional State (E_self)
        """
        with torch.no_grad():
    ...
    def infer_M1_other(self, observed_action: torch.Tensor, context_vector: Optional[torch.Tensor] = None):
        ...
        Наблюдая действие O(t), обновляем M1(t).
         Observing action O(t), we update M1(t).
        
        Theory Section 19b & 19e:
        p(M_other | O_behavior) ∝ p(O_behavior | M_other) · p(M_other) (Eq. 886)
        
        Generative Model (Eq. 881):
        p(O_behavior | M_other) ...
        """
        with torch.no_grad():
    ...
    def update_M2_recursion(self, my_last_action: torch.Tensor):
        ...
        Update M2: How my actions affect the other's opinion of me?
        
        Theory Section 19e, Level 2:
        dM₂_self_in_other/dt = r₂(M₁_other(t), a_self(t), O_self_visible(t)) (Eq. 984)
        
        "Если я ударил его (action), он теперь думает, что я враг (M2 update)"
        "If I hit him (action), he now thinks I am an enemy (M2 update)"
        """
        with torch.no_grad():
    ...
    def calculate_social_pain(self) -> float:
        """
        Социальная боль (Раздел 21e / Section 21e):
        Расхождение между M2 (как меня видят) и Идеальным Я (или потребностью в принадлежности).
        Discrepancy between M2 (how I am seen) and Ideal Self (or need for belonging).
        
        Rejection(t) = M₂_self_in_other показывает "они меня не ценят" (Eq. 1135)
        -> Activation ACC (Anterior Cingulate Cortex) -> Physical Pain similarity
        """
