...
def dynamics_kernel(V, phase, spikes, refractory_timer, ATP, is_dead,
    ...
    # Mathematical Model Section 1 & 17:
    # ----------------------------------
    # Eq 1: Membrane Potential / Мембранный потенциал
    # τ_i · dV_i/dt = -V_i + R_i · I_total
    #
    # Eq 13: Phase Dynamics / Фазовая динамика
    ...
    # Eq 23: Phase-Dependent Spiking / Спайк с учетом фазы
    # x_i(t) = f(V_i - θ_i) · [1 + α_sync · cos(φ_i)]
    # (Implemented as effective threshold modulation)
...
class BioChemistry:
    ...
    def get_gaba_conductance_modifier(self) -> torch.Tensor:
        ...
        Returns GABA_A conductance modifier based on anesthesia level.
        
        Theory Section 17d:
        g_GABA_A^{anes}(t) = g_GABA_A · [1 + k_anes · [Anesthetic](t)] (Eq. 533)
        """
    ...
    def update(self, dt: float, stress_level: float, reward_prediction_error: float):
        ...
        # Eq 47-51: Neuromodulation
        # ω_i(t) modulated by DA/ACh
        # This update reflects global background levels.
...
class NeuralLayer:
    ...
    def compute_kuramoto_order(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Theory Section 1:
        R(t) · e^(iΨ(t)) = (1/N) · Σ_j e^(iφ_j(t)) (Eq. 27)
        Synchronized ⟺ R(t) > 0.8
        """
...
class SynapseMatrix:
    ...
    def update_plasticity(self, dt: float, 
        ...
        Theory Section 4: Plasticity with Metaplasticity
        
        Eq 68: STDP with Phase Dependence
        dw_ij/dt = η(θ_M) · [K(Δt, Δφ) · x_i · x_j]
        
        Eq 72: Phase Window K(Δt, Δφ)
        Max potentiation when spikes happen at similar phases (Δφ ≈ 0).
        """
        ...
        learning_rate = self.learning_rate_base * (1 + self.chem.dopamine)
        
        # 5. Apply Updates
        if self.is_sparse:
            # We update W_dense but sync to W_sparse lazily
            
            # Extract effective LR for spiking neurons [k]
            # self.theta_M is [N_post], we need [k]
            eff_lr_active = learning_rate / (self.theta_M[post_spike_indices] + 0.1)
            
            # Apply Depression first (Simplification: depression on active rows)
            # D = 0.1 * W * spikes
            # row_updates = lr * (delta_LTP - depression)
            # row_updates = lr * delta_LTP - lr * 0.1 * W[active]
            
            current_weights_active = self.W_dense[post_spike_indices, :]
            
            # Total delta for active rows
            row_updates = eff_lr_active.unsqueeze(1) * (delta_w_active - 0.1 * current_weights_active)
...
class PredictiveUnit:
    ...
    Математика (Theory Section 5):
    1. Prediction: μ_l = g(X_{l+1}, a) (Eq. 118)
    2. Prediction Error: ε_l = Π · (Input - μ_l) (Eq. 120)
    3. Variational Free Energy: F ≈ 0.5 * ε^T · Π · ε + KL_terms (Eq. 123)
    """
    ...
    def calculate_error_and_free_energy(self, bottom_up_input: torch.Tensor) -> float:
        ...
        free_energy = 0.5 * torch.sum(self.prediction_error ** 2)
...
class BrainHierarchy:
    ...
    def process_sensory_input(self, sensory_input: torch.Tensor, 
        ...
        self.levels[0].calculate_error_and_free_energy(sensory_input)
...
class ConsciousnessSimulator:
    ...
    def step(self, dt: float):
        ...
        self.sleep_manager.update(dt_hours=dt / 3600.0, 
        ...
        v1_unit = self.hierarchy.levels[0]
        v1_error = v1_unit.prediction_error
        v1_precision = v1_unit.precision
    ...
    def load_brain(self, path: str):
        ...
        # 6. Global Workspace & Social Integration
        self.gwt.step(dt, social_module=self.social)
        
        # 7. Affective System Update
        social_pain = self.social.get_social_pain_signal()
        self.affect.update(dt, total_free_energy, 
                        body_pain=social_pain, 
                        reward_signal=0.0,
                        simulation_time=self.simulation_time)

        # 8. Social Cognition Simulation (Wake only)
        if self.sleep_manager.current_stage == 'Wake':
            dummy_other_action = torch.randn(10, device=DEVICE)
            self.social.observe_other(dummy_other_action, dt=dt)
            
            current_action = self.body.get_action_vector()
            
            self.social.update_self(
                current_action, 
                self.affect.valence.item(), 
                self.affect.arousal.item()
            )
            
            
            self.social.recursive_update(dt)
        # 9. Environment Dynamics
        self.environment_target = 0.5 + 0.3 * math.sin(self.simulation_time * 0.5)

        # 10. Mirror Neuron System
        if self.sleep_manager.current_stage == 'Wake':
            dummy_obs = torch.ones(10, device=DEVICE) 
            # Inject into V2 (analogous to premotor/mirror areas)
            mirror_current = self.body.activate_mirror_neurons(
                dummy_obs, 
                target_size=self.hierarchy.levels[1].layer.N
            )
            self.hierarchy.levels[1].layer.I_mirror += mirror_current
...
# ==========================================
# 14. ВИЗУАЛЬНАЯ ДЕМОНСТРАЦИЯ (Unified)
# ==========================================
...
def run_demo_with_plots(
    ...
    for step in tqdm(range(total_steps), desc=f"🧠 Simulating ({experiment_mode})", ncols=100):
        ...
        soa_values.append(sim.body.sense_of_agency)
